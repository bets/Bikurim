// CONSTANTS
const sourceSheetId = "1hUllfU7bqfNBu42vl7M0tzm2In5YZ5TqdqIH7p2pBuQ";
...
function handleQueryResponse(re) {
     ...
     buildingBlocks = buildingBlocks.sort(function (a, b) {
          return b.val - a.val;
     });
     ...
     for (const item of buildingBlocks) {
          // cl(item);
          let node = createHtml(
               `<p><label>${item.key}</label><span>${item.val}</span></p>`
          );
